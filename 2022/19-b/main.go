...
func main() {
	...
	for planNum, plan := range blueprints {
		...
		fmt.Printf("Plan %d has best output of %d.\n%s\n", planNum, bestOutput, sprintStates(bestState[:]))
		result *= bestOutput
	}
...
func (s state) validBuild(bp *blueprint) [itemCount]bool {
	...
	for i := itemNothing + 1; i < itemCount; i++ {
		...
		highest := 0       // avoid creating too much of any item
		...
		if able && (i == itemGeode || s.inventory[i] < highest*2) {
			valid[i] = true
